const tableCustomer = {
  ...
  renderTableLayout() {
    ...
    tableLayout.className =
      "table table-responsive-lg table-collapse table-bordered table-hover table-vertical-middle";
  ...
  handleAddCustomer(element) {
    ...
    if (Name != "" && Age != "" && Address != "" && Country > 0) {
      ...
      let isDuplicateCustomer = this.JSONData.findIndex((customer) =>
        this.deepEqualObj(customer, newCustomer)
      );
    ...
    if (Object.keys(this.choosingItem).length !== 0) {
      let countryDropdown = this.countryData
        .map((item) => {
          return `<option value=${item.Id} ${
            item.Id == this.choosingItem.Country ? "selected" : ""
          }>${item.Name}</option>`;
        })
        .join("");
      ...
      customerRowHTML.innerHTML =
        ...
        ` 
              <tr>
                <td class="text-center">
                  <input type="text" class="filter-name" value='${
                    this.choosingItem.Name
                  }' data-customer-value=${this.choosingItem.Name}>
                </td>
                <td>
                  <input type="number" min=0 class="filter-age" value=${
                    this.choosingItem.Age
                  } data-customer-value=${this.choosingItem.Age}>
                </td>
                <td class="text-center">
                ...
                <td>
                  <input type="text" class="filter-address address-field" value='${
                    this.choosingItem.Address
                  }'>
                </td>
                <td class="text-center">
                  <input type="checkbox" class="filter-married" ${
                    this.choosingItem.Married ? "checked" : ""
                  }>
                </td>
                <td class="text-center">
  ...
  handleUISort(element, fieldName) {
    ...
    for (let key in this.JSONData[0]) {
      if (key !== this.sortBy.choose) {
        this.rootElement
          .querySelector(`th[data-sort=${key}]`)
  ...
  filterCustomer() {
    ...
    if (fieldMarried != 1) {
      this.dataShow = this.dataShow.filter(
        (item) => item.Married == (this.indeterminateCheckbox === 0 ? false : true)
      );
    }
